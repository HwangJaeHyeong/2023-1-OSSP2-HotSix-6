...
const GroupTimeTableScreen = () => {
  ...
  const handleEventPress = (index) => {
    ...
    const existingEventIndex = selectedEvents.findIndex((selectedEvent) => selectedEvent.index === index)
  ...
  const printSelectedEventsInfo = () => {
    ...
    selectedEvents.forEach((selectedEvent) => {
      ...
      const time_len = Math.round(moment.duration(end.diff(start)).asMinutes() / 30)
    ...
    const handleTimetable = (schedules) => {
      ...
      for (let i = 0; i < schedules[0].length; i++) {
        ...
        if (start !== null) {
          ...
          generatedEvents.push(createEvent(start, end, i, schedules[schedules.length - 1][i]))
        }
      }
        ...
        style={[
          styles.eventButton,
          {
            ...
            backgroundColor:
              item.value === 0
                ? `rgba(0, 0, 255, 0.2)`
                : `rgba(0, 0, ${255 - item.value * 10}, ${item.value / 10 + 0.2})`,
          },
      ...
      <View style={styles.grid}>
        ...
        <View style={styles.eventGrid}>
          {eventsFormatted.map((event, index) => (
            <React.Fragment key={index}>{renderGridItem({ item: event, index: index })}</React.Fragment>
          ))}
        </View>
      </View>
