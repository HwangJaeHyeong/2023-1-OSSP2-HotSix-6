...
const GroupScreen = ({ route }) => {
  ...
  useEffect(() => {
    ...
    axios.get(`${SERVER_URL}/group/get-group/?email=${email}`).then((response) => {
  ...
  const leaveGroup = async () => {
    if (selectedGroup) {
      try {
        ...
        const updatedGroups = groups.filter((group) => group.Group_Code !== selectedGroup.Group_Code)
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList data={groups} keyExtractor={(item) => item.Group_Code.toString()} renderItem={renderGroupItem} />
      <TouchableOpacity style={styles.JoinGroupButton} onPress={() => navigation.navigate('JoinGroup', { email })}>
        <Text style={styles.JoinGroupButtonText}>그룹 코드로 그룹 입장하기</Text>
      </TouchableOpacity>

      <TouchableOpacity style={styles.CreateGroupButton} onPress={() => navigation.navigate('Makegroup', { email })}>
