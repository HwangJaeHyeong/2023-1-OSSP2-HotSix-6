...
const GroupNoticeScreen = ({ route, navigation }) => {
  ...
  const loadNotices = () => {
    axios
      .get(`${B_SERVER_URL}/group-notices?groupcode=${groupcode}`)
      .then((response) => {
  ...
  const createNotice = () => {
    axios
      .post(`${B_SERVER_URL}/group-notices`, { title, content, groupcode })
      .then((response) => {
        setTitle('')
        setContent('')
        loadNotices()
  ...
  return (
    <View style={styles.container}>
      <FlatList data={notices} renderItem={renderNoticeItem} keyExtractor={(item) => item.id.toString()} />
      ...
      {/* 생성 모달 창*/}
      <Modal animationType="slide" visible={createModalVisible} onRequestClose={() => setCreateModalVisible(false)}>
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            <Text style={styles.modalText}>제목</Text>
            <TextInput style={styles.modalTitle} value={title} onChangeText={(text) => setTitle(text)} />
            <Text style={styles.modalText}>내용</Text>
            <TextInput
              style={styles.modalInput}
              value={content}
