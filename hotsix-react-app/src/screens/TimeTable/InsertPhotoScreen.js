  ...
  const sendImageToServer = async () => {
    if (selectedImage && selectedHour && selectedMinute) {
      ...
      try {
        ...
        // 첫 이미지 보내기
        const response = await axios.post(`${A_SERVER_URL}/user/images/`, formData, config)

        // DB에 이미지 올리기 요청
        const Response = await axios.post(`${A_SERVER_URL}/user/img-time-table/`, { email: 'test1@test.com' })
        ...
        if (Response.status === 200) {
          Alert.alert('이미지와 시간 전송 성공', '이미지와 시간이 서버로 전송되었습니다.')
          navigation.navigate('Timetable')
        }
      } catch (error) {
        Alert.alert('이미지와 시간 전송 실패', '이미지와 시간을 서버로 전송하는 데 실패했습니다.')
      }
    } else {
      Alert.alert('이미지 및 시간 선택', '전송할 이미지와 시간을 선택해주세요.')
    }
  }
      ...
      {selectedImage && (
        <View style={styles.imageContainer}>
          ...
          <View style={styles.questionBubble}>
            <Text style={styles.questionText}>일정에서 제일 빠른 시작시간을 입력해주세요.</Text>
          </View>
          <Picker
